...
async function loadConfig() {
  ...
  document.getElementById('textoCancelamento').value = state.config.texto_cancelamento || TEXTOS_DEFAULT.cancelamentos;
...
function renderPreview() {
  ...
  const textoCanc = (state.config.texto_cancelamento||TEXTOS_DEFAULT.cancelamentos).split('\n').map(l=>l?`<p>${l}</p>`:'').join('');
  const tx = o.textos || {};
  const T = {
