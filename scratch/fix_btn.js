...
const updateActionsLogic = `window.atualizarBotoesCotacao = function() {
    ...
    if (existingCotacao) {
        actionsDiv.innerHTML = \`
            <button class="btn-secondary" onclick="abrirOrcamento('\${existingCotacao.id}'); navToPage('orcamento');" title="Visualizar a cotação existente sem alterar nada">👁️ Ver Cotação</button>
...
roteirosJS = roteirosJS.replace(endOfRenderDias, match => match.replace('}', '  if(window.atualizarBotoesCotacao) window.atualizarBotoesCotacao();\n}'));
