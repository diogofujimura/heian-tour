...
function emptyOrc() {
  return { id: null, nome: '', cliente: { nome: '', pessoas: '', dataOrcamento: '' }, estadias: [], consultoria: { ativa: false, valor: 0, descricao: '' }, tours: [], transportes: [], experiencias: [], itensAdicionais: [], textos: {}, criadoEm: null, atualizadoEm: null };
}
...
const TEXTOS_DEFAULT = {
  ...
  condicoes: `• O pagamento do sinal de 30% do valor dos TOURS é necessário para travar as datas e formalizar a contratação.\n• Conta Wise: @deborahm1346\n• Conta Brasileira: Banco 0260 Nu Bank / Ag 0001 / Cc 3510 / Deborah Monteiro Fujimura Lipka\n• O pagamento dos transportes e experiências deve ser feito integralmente entre 40 e 30 dias antes das datas.\n• O saldo de 70% dos tours deve ser pago na semana da viagem.`,
  cancelamento: `• Adiamento com mais de 45 dias: valores podem variar; taxa de ¥15.000 para mudanças significativas no roteiro.\n• Adiamento com menos de 45 dias: taxa adicional de ¥50.000 sobre os serviços, mais ¥15.000 para mudanças significativas.\n• Cancelamento total: o sinal de 30% sobre os tours não é estornado. Valores de transportes e experiências já comprados seguem política do fornecedor.`
};
...
async function loadConfig() {
  ...
  document.getElementById('textoObservacoes').value  = state.config.texto_observacoes  || TEXTOS_DEFAULT.observacoes;
  document.getElementById('textoCondicoes').value    = state.config.texto_condicoes    || TEXTOS_DEFAULT.condicoes;
  document.getElementById('textoCancelamento').value = state.config.texto_cancelamento || TEXTOS_DEFAULT.cancelamento;
  document.getElementById('sugestoesTours').value = state.config.sugestoes_tours || '';
  document.getElementById('sugestoesTransportes').value = state.config.sugestoes_transportes || '';
  document.getElementById('sugestoesExperiencias').value = state.config.sugestoes_experiencias || '';
...
function autoSave() {
  // Salva automaticamente em segundo plano, sem toast
  ...
  _autoSaveTimer = setTimeout(() => {
    ...
    const nome = document.getElementById('orcNome').value.trim()
      ...
      || 'Rascunho';
    // Só cria/atualiza se houver algum conteúdo
...
function abrirOrcamento(id) {
  ...
  if (document.getElementById('orcRoteiroVinculado')) {
    document.getElementById('orcRoteiroVinculado').value = orc.roteiroVinculado || '';
  }
...
function setupNav() {
  ...
  if (hash && document.getElementById('page-' + hash)) {
    ...
    if (hash === 'orcamento') novoOrcamento();
    else navToPage(hash);
  } else {
  ...
  state.orcamento.tours.forEach((t, i) => {
    ...
    div.innerHTML = `
      <div class="item-row-header">
        ...
        <span class="item-subtotal" id="subtotal-tour-${t.id}">¥${fmt(t.valor)} · ${fmtUSD(t.valor * getUSD())}</span>
      ...
      <div class="form-grid">
        ...
        <div class="field"><label>Duração do Tour</label>
          <input type="text" value="${t.duracao||''}" placeholder="Ex: 8 horas" oninput="updTourField(${t.id},'duracao',this.value)"></div>
        <div class="field"><label>Valor do Tour ¥ (já com desconto, se houver)</label>
          <input type="number" value="${t.valor||''}" placeholder="Ex: 80750"
...
function updTourNum(id, f, rawVal) {
  ...
  if (el) el.textContent = `¥${fmt(t.valor)} · ${fmtUSD(t.valor * getUSD())}`;
  updateResumo();
}
...
function addTransporte() {
  state.orcamento.transportes.push({ id: Date.now(), data: '', descricao: '', preco: 0, pessoas: 2, taxaAtiva: false, taxaTipo: 'grupo', taxaValor: 3000, observacao: '', _dbId: null });
  renderTransportesForm(); updateResumo();
}
function calcTotalTransporte(t) {
  let base = (t.preco||0) * (t.pessoas||1);
  if (t.taxaAtiva) base += t.taxaTipo === 'grupo' ? (t.taxaValor||3000) : (t.taxaValor||3000) * (t.pessoas||1);
  return base;
}
function renderTransportesForm() {
  ...
  state.orcamento.transportes.forEach((t, i) => {
    ...
    const taxaVal = t.taxaAtiva ? (t.taxaTipo==='grupo' ? t.taxaValor : t.taxaValor*t.pessoas) : 0;
    const opts = state.transportesDB.map(tr =>
      `<option value="${tr.id}" ${t._dbId==tr.id?'selected':''}>` +
      `${tr.trecho} | ${tr.tipo} | ${tr.linha} | ${tr.categoria} — ¥${fmt(tr.preco_jpy)}</option>`
    ).join('');
    const div = document.createElement('div');
    div.className = 'item-row';
    div.innerHTML = `
      <div class="item-row-header">
        ...
        <span class="item-subtotal" id="subtotal-transp-${t.id}">¥${fmt(t.preco)} × ${t.pessoas}${t.taxaAtiva?` + taxa`:''}  =  ¥${fmt(total)} · ${fmtUSD(total*getUSD())}</span>
      ...
      <div class="form-grid">
        ...
        <div class="field"><label>Nº Pessoas</label>
          <input type="number" value="${t.pessoas}" min="1"
            oninput="updTranspNum(${t.id},'pessoas',this.value)"
            onblur="updTranspRefresh(${t.id},'pessoas',this.value)"></div>
        <div class="field full-width">
          ...
          <div class="taxa-row" style="margin-top:6px">
            ...
            <input type="number" value="${t.taxaValor}" style="width:110px${t.taxaAtiva?'':';opacity:0.4;pointer-events:none'}"
...
function updTranspNum(id, f, rawVal) {
  ...
  if(taxaEl) taxaEl.textContent = t.taxaAtiva ? (`= ¥${fmt(t.taxaTipo==='grupo'?t.taxaValor:t.taxaValor*t.pessoas)}`) : '';
  updateResumo();
}
...
function preencherTransporte(id, dbId) {
  ...
  if(t&&db){ t._dbId=db.id; t.descricao=`${db.trecho} | ${db.tipo} | ${db.linha} | ${db.categoria}`; t.preco=db.preco_jpy; t.observacao=db.observacao||''; }
...
function propagarPessoas() {
  ...
  state.orcamento.transportes.forEach(t => { t.pessoas = num; });
...
function updateResumo() {
  autoSave();
  const tT  = state.orcamento.tours.reduce((s,t)=>s+(t.valor||0),0);
...
function renderTabelaTransportes(filtro='') {
  ...
  tbody.innerHTML = lista.map(t=>`<tr><td>${t.trecho}</td><td>${t.tipo}</td><td>${t.linha}</td><td>${t.categoria}</td><td class="preco-cell">¥${fmt(t.preco_jpy)}</td><td>${t.tempo||'—'}</td><td><button class="btn-icon" onclick="abrirModalTransporte(${t.id})">✎</button> <button class="btn-icon" onclick="deletarTransporte(${t.id})">✕</button></td></tr>`).join('');
}
...
async function salvarTransporte(id) {
  const dados={trecho:v('m_trecho'),tipo:v('m_tipo'),linha:v('m_linha'),categoria:v('m_categoria'),preco_jpy:parseFloat(v('m_preco'))||0,tempo:v('m_tempo'),observacao:v('m_obs'),link:v('m_link')};
  ...
  document.getElementById('btnSync').addEventListener('click', async () => {
    ...
    try{const res=await fetch('/api/sync',{method:'POST'});const data=await res.json();
      if(data.ok){
        ...
        await loadDB();
        showToast(`Base atualizada! ${data.nTransp||0} transportes, ${data.nExp||0} experiências, ${data.nAtracoes||0} atrações.`);
      }
...
function renderPreview() {
  ...
  const tT  = o.tours.reduce((s,t)=>s+(t.valor||0),0);
  ...
  const transpRows = o.transportes.map(t=>{
    ...
    return `<tr><td>${t.data?fmtDataBR(t.data):'—'}</td><td><div class="pdf-desc-trecho">${trecho}</div>${sub?`<div class="pdf-desc-sub">${sub}</div>`:''} ${t.observacao?`<div class="pdf-desc-obs">${t.observacao}</div>`:''}</td><td class="num">¥${fmt(t.preco)}</td><td class="num">${t.pessoas}</td><td>${taxaLabel}</td><td class="num">¥${fmt(total)}</td><td class="num">${fmtUSD(total*usd)}</td></tr>`;
  }).join('');

  const tourRows = o.tours.map(t=>{
    const pontos=t.pontos?t.pontos.split('\n').filter(Boolean).map(p=>`<span class="pdf-ponto-box">${p}</span>`).join(''):'';
    const duracaoLabel = t.duracao ? ` · ${t.duracao}` : '';
    return `<tr><td>${t.data?fmtDataBR(t.data):'—'}</td><td><div class="pdf-desc-trecho">${t.descricao||'—'}${duracaoLabel}</div>${pontos?`<div class="pdf-pontos-wrap">${pontos}</div>`:''} ${t.observacao?`<div class="pdf-desc-obs">${t.observacao}</div>`:''}</td><td class="num">¥${fmt(t.valor)}</td><td>${t.descontoAtivo&&t.desconto?t.desconto+'%':'—'}</td></tr>`;
  }).join('');
  ...
  const textoObs  = (state.config.texto_observacoes ||TEXTOS_DEFAULT.observacoes).split('\n').map(l=>l?`<p>${l}</p>`:'').join('');
  const textoCond = (state.config.texto_condicoes   ||TEXTOS_DEFAULT.condicoes).split('\n').map(l=>l?`<p>${l}</p>`:'').join('');
  const textoCanc = (state.config.texto_cancelamento||TEXTOS_DEFAULT.cancelamento).split('\n').map(l=>l?`<p>${l}</p>`:'').join('');
  const tx = o.textos || {};
  const T = {
...
function showPrintTip() {
  ...
  Object.assign(tip.style, {
    position:'fixed', bottom:'80px', right:'28px', background:'#1A1410', color:'white',
...
function showToast(msg) {
  ...
  Object.assign(t.style,{position:'fixed',bottom:'28px',right:'28px',background:'#8B1A2B',color:'white',padding:'12px 22px',borderRadius:'6px',fontSize:'13px',fontFamily:'Jost,sans-serif',letterSpacing:'0.04em',zIndex:9999,boxShadow:'0 4px 20px rgba(0,0,0,0.25)',transition:'opacity 0.4s'});
